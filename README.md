...
| 19 | Production hardening continued: real AWS Cost Explorer billing sync (moto-tested, no live AWS account available - disclosed); CD pipeline (`helm upgrade`, built and documented but disabled pending a real cluster's `KUBE_CONFIG`); SMS notification channel (Twilio, self-service phone number via `PATCH /auth/me`) | **In progress** - see [`docs/PHASE_19.md`](docs/PHASE_19.md) |
